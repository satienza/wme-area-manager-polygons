...
5.1. **Modo polígono libre**: el panel ofrece un selector de forma ("Rectángulo" / "Polígono"). En modo polígono, el usuario hace clic tantas veces como necesite para añadir vértices y cierra el trazado pulsando sobre el punto inicial, usando el método nativo `Map.drawPolygon()` del SDK (no se reimplementa la interacción de dibujo a mano).
   ...
   - **Zonas clicables sin ambigüedad** (Fase 6): el polígono se dibuja con tres features separadas — un relleno semitransparente de todo el interior, el trazo del contorno encima, y los marcadores de vértice encima de ambos — para que el hit-testing del propio SDK (no matemática propia) resuelva siempre el mismo clic a la misma acción, en vez de depender de un test geométrico que compite con el margen de clic del SDK sobre la línea.
   - **Añadir vértices** (Fase 6): un clic sobre el trazo del contorno (no sobre el relleno interior ni sobre un marcador de vértice) inserta un vértice nuevo en la arista más cercana al punto clicado.
   - **Arrastrar la figura completa** (Fase 6, sustituye a la implementación inicial del punto 8): un clic sobre el relleno interior semitransparente arma el arrastre — el relleno hace visible qué zona es clicable para esto. Ya no se usa un test geométrico propio (`pointInRing`) para decidir si un clic cae "dentro" del polígono.
   - **Mover un vértice** (Fase 6): un clic sobre su marcador arma su arrastre; el marcador cambia de color mientras está armado. El punto sigue al cursor y un segundo clic lo suelta — mismo modelo clic-clic que el arrastre de la figura completa, para no chocar con el paneo nativo del mapa.
   - **Borrado de vértices** (Fase 6): situarse sobre el marcador de un vértice y pulsar la tecla configurada (configurable desde el panel, Fase 7) lo elimina y redibuja el polígono — mismo convenio que usa el editor nativo de WME para geometrías de "places" de área. Sustituye al borrado por clic izquierdo de versiones anteriores, ya que el clic sobre el marcador pasa a usarse para arrastrarlo. No se permite bajar de 3 vértices. El SDK no expone un evento `keydown` nativo; el atajo se registra con `Shortcuts.createShortcut` (módulo del SDK dedicado a atajos de teclado de userscript). Se descarta igualmente el borrado por clic derecho: el SDK no expone qué botón del ratón se pulsó (`SdkMouseEvent` no tiene campo `button`) ni ofrece un evento de clic derecho sobre features; enganchar `contextmenu` del DOM del mapa por fuera de la API pública sería frágil y no está documentado.
   - Reutiliza el enlace al centro (punto 9) sobre un centro aproximado (media de los vértices) del polígono.
   - **Edición restringida al modo polígono**: añadir vértice, mover un vértice individual y borrar vértice son exclusivos de una forma guardada con `tipo: 'polygon'`. Un rectángulo (`tipo: 'rectangle'`) usa la misma capa y el mismo estilo visual (relleno/contorno verde o rojo) al editarlo, pero solo admite el arrastre de la figura completa — su tamaño lo fija el nivel del editor (punto 4), así que no debe poder reformarse vértice a vértice. El tipo se guarda junto al resto de la entrada (ver "Estructura de datos", sección 3) para que "Editar" (punto 7) sepa qué comportamiento aplicar.
...
7. **Gestión desde el panel**: el panel lateral muestra la lista de rectángulos guardados (nombre, nivel abreviado como "N" + número, fecha de creación). Por cada entrada:
...
8. **Reposicionable**: permitir arrastrar la figura (rectángulo o polígono) para reubicarla mediante una traslación que preserva el área física real, no un delta fijo de grados: el desplazamiento en longitud se reescala por `cos(latAncla)/cos(latActual)` (1° de longitud equivale a menos metros cerca de los polos que cerca del ecuador), mientras que el desplazamiento en latitud sí es un delta directo (1° de latitud es ~constante en cualquier punto). Si una figura guardada se arrastra, su nueva posición debe poder volver a guardarse (sobrescribiendo lat/lon de la entrada). Implementación: capa propia + arrastre manual (ver detalle técnico en sección 3). Se descarta la alternativa de usar un Venue temporal del modelo de datos por el riesgo de que acabe guardado como Place real en el mapa.
...
- Módulos usados: `State` (rank de usuario), `Map` (`drawPoint`, `drawPolygon`, `addLayer`, `addFeatureToLayer`, `removeFeatureFromLayer`, `getMapCenter`, `getZoomLevel`), `Sidebar` (panel de control), `Events` (`wme-ready`, `wme-layer-feature-clicked`/`wme-layer-feature-mouse-enter`/`wme-layer-feature-mouse-leave` vía `trackLayerEvents` para añadir/mover/detectar el vértice bajo el cursor, eventos de ratón para el arrastre), `Shortcuts` (`createShortcut`, atajo `d` para borrar el vértice bajo el cursor — Fase 6).
...
- **Estructura de datos por rectángulo guardado**:
  `{ id, nombre, tipo, lat, lon, nivel, zoom, area_km2, env, fechaCreacion, geometry }`
  Identificador `id` único (no el nombre) para permitir nombres duplicados y renombrados sin romper referencias. `geometry` es el `GeoJSON.Polygon` realmente trazado (rectángulo o polígono libre), no recalculado a partir de nivel/lat/lon, para poder reproducir polígonos libres tal cual y exportarlos a WKT/GeoJSON (ver punto 7). `tipo` (`'rectangle'` | `'polygon'`) determina si "Editar" (punto 7) permite edición de vértices o solo arrastre de la figura completa (ver punto 5.1). Las entradas guardadas antes de introducir este campo no lo tienen (`undefined`) y por defecto se tratan como editables, igual que su comportamiento antes de este cambio.
