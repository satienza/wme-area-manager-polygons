...
6. **Guardado con nombre**: tras trazar el rectángulo, un campo de texto ("Nombre") y un botón "Guardar" lo añaden a una lista persistente. El nombre es obligatorio para guardar; no se guarda automáticamente al dibujar.
...
9. **Enlace al centro**: generar y mostrar/copiar un enlace con formato:
...
- Módulos usados: `State` (rank de usuario), `Map` (`drawPoint`, `addLayer`, `addFeatureToLayer`, `removeFeatureFromLayer`, `getMapCenter`, `getZoomLevel`), `Sidebar` (panel de control), `Events` (`wme-ready`, eventos de ratón si se implementa el arrastre).
